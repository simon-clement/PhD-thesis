...
will be parameterized with turbulence closure schemes
...
drastically the complexity of the system but keeps reasonable values
for the variables.
\par
...
\langle p \rangle$: a \textit{turbulence closure}
...
energy of the mean velocity $\displaystyle\frac{\langle u \rangle^2}{2}$.
...
$N^2 = - \displaystyle \frac{g\partial_z \rho}{\rho_0}$ is the Brunt-Väisälä
frequency.
...
be introduced in \S\ref{sec:airseaSCM_hierarchy_TurbulentEkman}) and
...
u_1(0^-,t) &=  u_2(0^+,t) &\qquad& t \in [0,T] \label{eq:airseaSCM_interface-dir} \\
...
previous iteration ${\mathcal B}_2 u_2^{k-1}(0^+,t)$:
\begin{subequations}
\begin{align}
\partial_t u^k_1 +( r - \nu_1 \partial_x^2) u^k_1 &= f_1  &\qquad& (x,t) \in (-\infty,0) \times ]0,T]  \\
u_1^k(x,0) &= u_{1,0}(x)   &\qquad&  x \in (-\infty,0)  \\
{\mathcal B}_1 u_1^k(0^-,t) &= {\mathcal B}_2 u_2^{k-1}(0^+,t) &\qquad& t \in [0,T] 
\end{align}
...
\begin{subequations}
\begin{align}
\partial_t u^k_2 + ( r - \nu_2 \partial_x^2) u^k_2  &= f_2  &\qquad& (x,t) \in (0,+\infty) \times ]0,T] \\
u^k_2(x,0) &= u_{2,0}(x)   &\qquad&  x \in (0,+\infty) \\
{\mathcal C}_2 u_2^k(0^+,t) &= {\mathcal C}_1 u_1^{k}(0^-,t) &\qquad& t \in [0,T] \label{eq:airseaSCM_SWR-for-u2-interface}
\end{align}
\label{eq:airseaSCM_SWR-for-u2}
\end{subequations}
The convergence is attained
when ${\mathcal B}_1 u_1^k(0^-,t) = {\mathcal B}_2 u_2^{k}(0^+,t)$
...
\begin{enumerate}
	...
	\item Use the transmission operators to derive the
		...
		|\frac{\widehat{e}^k_j(0,\omega)}
		{\widehat{e}^{k-1}_j(0,\omega)}|$.
\end{enumerate}
...
K_u \partial_z u_o^k(\delta_o) &=
	K_u \frac{\rho_a}{\rho_o} \partial_z u_a^k(\delta_{\rm sl})
	& t \in ]0,T] \label{eq:airseaSCM_bd1} \\
	|K_u \partial_z u_a^k(\delta_{\rm sl})|
	&= {\rm BULK}(u_a^k(\delta_{\rm sl})
	-u_o^{k-1}(\delta_o))^2,
...
\begin{equation}
	...
	\begin{aligned}
	\nu_a \rho_a \partial_z U_a^k(\delta_{\rm sl})
		&= C_D \left|U_a^{k-1}(\delta_{\rm sl})
		-U_o^{k-1}(\delta_o)\right|\left(U_a^{k-1+\theta}
		(\delta_{\rm sl}) -U_o^{k-1}(\delta_o)\right)
	\end{aligned}
\end{equation}
...
condition.
% \subsubsection*{Discretizations FV}
