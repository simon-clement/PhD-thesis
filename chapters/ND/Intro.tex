...
domain is justified by the lack of numerical precision
...
first grid level.
However, the evolution equation \eqref{eq:ND_NeutralCase_EkmanEq}
is also solved inside the surface layer which contradicts the
quasi-stationary hypothesis.
we can identify three ways to improve
the treatment of the surface layer (subjectively in decreasing
...
\begin{enumerate}
	...
	\item It is expected from most discretizations to be
		...
		is not the case when $\delta_{sl}$ is the first grid
