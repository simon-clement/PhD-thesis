...
$\delta_{\rm sl}$ in the \textbf{continuous neutral} model.
...
\item the viscosity $K_u(z)$ for $z \in [\delta_{1}, \delta_{2}]$
	is forced by the law of the wall to compute $u_2$
...
\begin{itemize}
	\item Even if the bulk algorithm is well designed and returns
		...
		in the general case
		${\rm BULK}_1(u_1(\delta_1)) \neq
		{\rm BULK}_2(u_2(\delta_1))$ hence the friction scales
...
in $u_\star$ and $u$
between high and low resolutions is far smaller when using
the "FV free" scheme. Unlike the previous cases, the "FV2"
scheme do not show a lot of differences between
its high and low resolutions.
Finally, the dependency of $t_\star$ on the resolution
seems
