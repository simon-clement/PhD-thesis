...
and the wall law $||u^n(\delta_{sl})|| = \frac{{u_\star}}{\kappa}\ln(1+\frac{\delta_{sl}}{z_{u}})$ in \eqref{eq:ND_NeutralCase_Chasles}.
One then gets:
\begin{equation}
...
\ln(1+\frac{\delta_{\rm sl}}{z_{u}}) - \delta_{\rm sl}}{
	h_{1/2}\ln(1+\frac{\delta_{sl}}{z_{u}})}
\end{equation}
...
increasing $\delta_{sl}/z_1$ or $z_u$.
\begin{figure}
	\centering
...
which will vary.
On the other hand, we assume that $\delta_{\rm sl}$ does not change
over time. Indeed, it would be challenging to decrease
$\delta_{\rm sl}$ during the integration in time because a part
of the wall law would need to be treated as a spline.
It would be however desirable to be able to vary $\delta_{\rm sl}$
