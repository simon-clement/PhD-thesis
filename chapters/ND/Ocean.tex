...
% 	\frac{\exp(k_i x)}{-x + z_{u}}
% 	- \exp(k_i x)\frac{1 - \phi_0^h(\frac{-x}{L_{MO}})}
% 	{-x + z_{u}}
% 	dx
% \end{equation}
% \begin{equation}
% 	E(z) = \sum_i A_i \exp(k_i z_{u}) \left(E1(k_i z_{u})
% 	- E1(k_i (-z+z_{u)})\right)
% 	- \int_{z}^0 \sum_i A_i \exp(k_i x)
% 	\frac{1-\phi(\frac{-x}{L_{MO}})}{-x+z_{u}} dx
% \end{equation}
% where $E1=\int_x^\infty \frac{\exp(-t)}{t}dt$ is the exponential
% integral.
% The left part $\sum_i A_i \exp(k_i z_{u}) \left(E1(k_i z_{u})
% 	- E1(k_i (-z+z_{u)})\right)$ should be smaller than
% $\int_z^0 \frac{1}{-x + z_{u}}dx$
