...
\end{aligned}
\end{equation}
{\color{red} TODO regarder forcing dans toutes les equations d'après}
The viscosities $K_u, K_\theta$ will be detailed in section
\ref{sec:ND_StratifiedCase_turbulentVisc}.
The boundary condition on the fluxes $K_u \partial_z u$ and
...
-
    ...
    \right)
&= \overline{F_{\theta, m+1/2}} - \overline{F_{\theta, m-1/2}}
\end{aligned}
\end{equation}
\subsection{FV free}
...
We assume that
\eqref{eq:ND_StratifiedCase_MOST} applies for $z<\delta_{sl}$,
which leads us to separate the 
cell $[z_k, z_{k+1}]$
into two parts: the surface layer part $[z_k,\delta_{\rm sl}]$ and the
...
(\partial_t + if)
    ...
    \right)
	    \phi_{k+1}
	    + \frac{h_{k+3/2}}{6} \phi_{k+2}\right)
    = \\
	    \frac{K_{u, k+2} \phi_{k+2} - K_{u, k+1} \phi_{k+1}}
	    {h_{k+3/2}} - \frac{K_{u, k+1} \phi_{k+1} -
	    K_{u,k} \phi_k }{\widetilde{h}}
    \end{aligned}
\end{equation}
\begin{equation}
    \begin{aligned}
...
\label{eq:ND_StratifiedCase_prognosticPT_FVfree}
    \begin{aligned}
	    &\partial_t \left(\frac{\widetilde{h}}{6} 
	    {(\partial_z \theta)}_k
    +
    \left(
	    \frac{\widetilde{h}}{3} 
    ...
    \right)
	    ...
	    + \frac{h_{k+3/2}}{6} {(\partial_z \theta)}_{k+2}\right)
    = \\
	    &\frac{K_{\theta, k+2} {(\partial_z \theta)}_{k+2} -
	    K_{\theta, k+1} {(\partial_z \theta)}_{k+1}}
	    {h_{k+3/2}} - \frac{K_{\theta, k+1} {(\partial_z \theta)}_{k+1} -
	    K_{\theta, k} {(\partial_z \theta)}_k }{\widetilde{h}}
	    + \overline{F_{\theta, k+3/2}} -
	    \overline{F_{\theta, k+1/2}}
    \end{aligned}
\end{equation}
\begin{equation}
    \begin{aligned}
...
$l_m, l_\epsilon$ are obtained with the fourth method described in \cite{lemarie_simplified_2021} (section 4.2.2) named $l^\star_{\rm D80}$.
...
\cite{lemarie_simplified_2021} $l_{up}$ is limited by
...
\eqref{eq:ND_StratifiedCase_solution_l_down} to compute $l_{down}$.
