...
\subsection{FV free}
{\color{red} TODO simplifier cette section}
...
\begin{equation}
	...
	\left(\overline{\theta}_{k+1/2} + \widetilde{h}\tau_{\rm sl, \theta}
	(\frac{{(\partial_z \theta)}_k}{3} + \frac{{(\partial_z \theta)}_{k+1}}{6})\right)\right)
	= \frac{K_{\theta, k+1}{(\partial_z \theta)}_{k+1} - K_{\theta,k} {(\partial_z \theta)}_k}{\widetilde{h}}
\end{equation}
