...
\item
	...
	% 	\partial_z u = u_\star \frac{\phi_m(z/L_{MO})}{\kappa (z + z_{u})}\frac{u(\delta_{sl})-u(0)}{|u^n(\delta_{sl})-u(0)|}
	% \end{equation}
	% \begin{equation}
	% 	\label{eq:ND_StratifiedCase_partial_z_theta}
	% 	\partial_z \theta = \theta_\star \frac{\phi_h(z/L_{MO})}{\kappa (z + z_{u})}, ~~~ K_\theta \partial_z \theta = u_\star \theta_\star
	% \end{equation}
	% \end{subequations}
	% Leading to 
