...
used in realistic models \cite{Marti_etal_2020}. For such 
...
using wall laws a.k.a. log laws (e.g. \cite{Mohammadi_etal_1998}). This approach, 
traditionally used to deal with solid walls, is also used
in the ocean-atmosphere context, with additional complexity 
arising from the stratification effects \cite{Pelletier_etal_2021}.
...
\cite{Thery_2021}
%\cite{Thery_2021} studied the well-posedness of \eqref{eq:cplProblem}. 
...
As discussed for example in \cite{Marti_etal_2020},
...
by $U_{\rm a}^k = \theta U_{\rm o}^{k-1} + (1-\theta) U_{\rm a}^{k-1}$ the DNWR algorithm is retrieved, as examined in the continuous case in \cite{kwok2013} and in the discrete case in
\cite{mongeMultirate2021}.
...
Whenever $f=0$, the solution of this problem is 0 because of the infinite domain hypothesis and of the formulation of this problem. We hence interest us only on the case when $f\neq 0$. In this section, we assume that the
space domain is sufficiently large ($H_j \rightarrow \infty$) but the results can be easily extended to finite domains.
...
frequency domain (e.g. \cite{Wu2017}) leads to 
...
The stationary state analysis is identical, except that $\omega$ is set to 0 and that it does not requires the transmission conditions to be linear. 
...
\begin{remark}[Sophie's notations, \S 5.1.3, \S 5.1.4]
...
\end{remark}
We interest ourselves now to the real and imaginary parts: $\widetilde{x}=\widetilde{x}_R+i{\widetilde{x}_I}$ and $d={d_R}+i{d_I}$.
\begin{equation}
    \begin{aligned}
...
that the space domain is of infinite size (i.e. $H_j\to\infty$) for the 
sake of simplicity.
%====================================================
...
cannot be pursued through a Fourier transform.
...
we find that the evolution of $B_k$ is: (warning, conjugate of Z transform is Z transform of conjugate with conjugate $z$)
\begin{equation}
...
quadratic friction case hence differs from one iteration to another 
(it is indeed a function of $\frac{\overline{B_{k-1}(-\omega)}}{B_{k-1}(\omega)}$).
\paragraph{Asymptotic ($\omega \rightarrow 0$) convergence}
...
here $\theta_{\rm opt}^q = \frac{3}{2} + \frac{3}{2}\epsilon\sqrt{\frac{\nu_{\rm a}}{\nu_{\rm o}}} $.
It is different from the optimal parameter $\theta_{\rm opt}$ obtained with linear friction: for typical values of the ocean-atmosphere coupling problem, $\theta_{\rm opt}^q$ is close to $\frac{3}{2}$. The asymptotic value $\xi_0^q$ is not an upper bound of the convergence factor but it is  a good choice 
for $\theta_{\rm opt}^q$.

\paragraph{Non-asymptotic convergence}
We need to look at the $4\times 4$ transition matrix: let $z\in\mathbb{C}$, let $a_{1r}, a_{1i}, a_{2r}, a_{2i}, a_{3r}, a_{3i}, a_{4r}, a_{4i} \in \mathbb{R}$ 
such as 
\begin{equation}
\begin{aligned}
    B_{k+1}(z) &= (a_{1r} + i a_{1i}) B_k(z)
    + (a_{3r} + i a_{3i}) \overline{B_k(\overline{z})} \\
    \overline{B_{k+1}(\overline{z})} &= (a_{2r} + i a_{2i}) \overline{B_k(\overline{z})}
    + (a_{4r} + i a_{4i}) B_k(z)
\end{aligned}
\end{equation}
\begin{equation}
\begin{pmatrix}
\Re (B(z)) \\ \Im (B(z)) \\ \Re(B(\overline{z})) \\ -\Im(B(\overline{z}))
\end{pmatrix}_{k+1}
 =
\begin{pmatrix}
a_{1r} & - a_{1i} & a_{3r} & - a_{3i} \\
a_{1i} & a_{1r} & a_{3i} & a_{3r} \\
a_{4r} & a_{4i} & a_{2r} & a_{2i} \\
-a_{4i} & a_{4r} & - a_{2i} & a_{2r}
\end{pmatrix}
\begin{pmatrix}
\Re (B(z)) \\ \Im (B(z)) \\ \Re(B(\overline{z})) \\ -\Im(B(\overline{z}))
\end{pmatrix}_{k}
...
\begin{aligned}
    ...
    \lambda_{\rm a}
    + \frac{3}{2} \epsilon \lambda_{\rm o} \right)}{\left(\frac{(s+if)h_{\rm a}}{\alpha} - \theta \lambda_{\rm a}\right)}
    \\
    (a_{2r} - i a_{2i})(z)
    &=
    (a_{1r} + i a_{1i})(\overline{z})
    \\
    (a_{3r} + i a_{3i})(z)
    &= \frac{
 \frac{
...
also displayed: $\sup_\omega \xi$ is an upper bound of the $L^2$ convergence factor \cite{Thery_2021} and $\xi_0$ is an approximation of $\sup_\omega \xi$.
