...
\myTD{réécrire ici (voir commentaires eric)}
\par
...
well-posedness of \eqref{eq:SWRbulk}.
...
\myTD{rappeler le inverse function theorem}
We will follow their method which consists of three steps:
\myTD{faire la lien continu -> semi-discret}
\begin{enumerate}
...
\myTD{itemize pour clarifier les choses entre finite/infinite size}
...
find an equation roughly similar to
\myTD{pas d'équation pour le futur}
\eqref{eq:mainEqSimplified}.
...
The steady state analysis is similar,
except that $\omega$ is set to 0 and that
the transmission conditions are not required to be linear. 
We note $\widetilde{U_j}(z)$ the stationary solution
(and $\widetilde{\phi}_j$ its finite difference derivative)
and the space discretization at the first grid level
$\frac{h_j}{2}$ gives:
$\widetilde{U}_j(\delta_j) =
g_j \pm \frac{\nu_j\lambda_j}{if h_j} \{A_k^{\phi}, B_k^{\phi}\}_j$
\myTD{Notation pas claire}
\begin{remark}[\cite{thery_etude_2021}, \S 5.1.3, \S 5.1.4]
We retrieve a solution of the form
$\widetilde{U}_a(\delta_a) = g_a - \Psi_a (u^\star)^2 e^{i\theta}$
but $\Psi_j = -\frac{\lambda_j}{if h_j}$ is simpler thanks to the hypotheses of constant diffusivities and infinite domains.
\end{remark}
\myTD{clarifier: c'est dans le cas continu. La remarque s'applique
au cas continu défini via $\Omega_j\cup \Omega_{sl}$?
+ bizarre de donner $U(\delta_a)$ sans réf explicite à $\delta_j$}
\subsubsection{Simplification of the problem}
\myTD{commencer par dire quel problème on essaie de simplifier}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
$1 - |\widetilde{x}| d_R$ should not be zero as it would require that $g_{\rm a} - g_{\rm o} = \pm \frac{d_I}{d_R^2}$ ($d$ does not depend on $g_j$ and the latter values are arbitrarily chosen in this study:
...
corresponds to the solution of the semi-discrete,
nonlinear problem. \myTD{citer le problème couplé ?}
...
\myTD{introduire un peu plus là}
...
steady state. \myTD{formuler explicitement en 6.3.1}
\begin{enumerate}
	\item The existence of a steady state $U^e$ is discussed above
		in this section. \myTD{citer section}
	...
	\begin{enumerate}
		...
		where $\mathbf{\cal U} = L^2([0,T])^{M_a+M_o+1}$.
			\myTD{définir $M_a, M_o$ et 
			dire explicitement pourquoi $\phi_o$ ne fait
			pas partie de U (voir premier item en dessous,
			$\phi_o$ est implicite)}
		\item
	Define a mapping
...
\end{remark}
\myTD{visualisation $\xi$ pour certaines valeurs de $\theta$}
...
\myTD{citer un autre système qui correspond au steady state}
...
\begin{equation}
	...
	\right)
\end{equation}
where $\langle D_x ??, \mu \rangle$ denotes the total derivative
of the application ?? at x applied on $\mu$.
\myTD{définir les crochets de dualité mieux que ça}
...
\begin{figure}[htpb]
	...
	\myTD{a faire en dernier:
	voir si la footnote est au bon endroit.
	Aussi, attention aux figures qui arrivent trop tôt}
	of eigenvectors
