...
\begin{equation}
	...
	\theta \lambda_{\rm a}\right)B_{k+1}(z)
    =
...
\paragraph{Asymptotic ($\omega+f \rightarrow 0$) convergence}
...
rate $\xi_0^q$ above
because $B_{k+1}(i\omega)$ depends on
$\overline{B_k(- i\omega)}$ which could explode because
$|\overline{\mathcal{O}(\omega\rightarrow f)}| \rightarrow \infty$.


\paragraph{Non-asymptotic convergence}
We need to look at both $B_{k+1}(z), \overline{B_{k+1}(\overline{z})}$
at the same time: let $z\in\mathbb{C}$,
let $a_1, a_2 \in \mathbb{C}$ such as
\begin{equation}
	\begin{aligned}
		B_{k+1}(z) &= a_1 B_{k}(z) + a_2
		\overline{B_{k}(\overline{z})}\\
		\overline{B_{k+1}(\overline{z})} &=
		\overline{a_1(\overline{z})
			B_{k}(\overline{z})
		+ {a_2(\overline{z})} \overline{B_{k}(z)}}
	\end{aligned}
\end{equation}
...
\begin{pmatrix}
	a_1(z) & a_2(z) \\
	\overline{a_2(\overline{z})} & \overline{a_1(\overline{z})} \\
\end{pmatrix}
\begin{pmatrix}
	B(z) \\
	\overline{B(\overline{z})}
...
\end{aligned}
\end{equation}
The biggest singular value of the $2\times 2$ matrix
