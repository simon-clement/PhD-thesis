...
$\widehat{\phi}_{{\rm o}, -m}^k = A_k (\lambda_{\rm o}+1)^m$
and
$\widehat{\phi}_{{\rm a}, m}^k = B_k (\lambda_{\rm a}+1)^m$ 
...
\myTD{TODO replace $\mu(s)$
et relire la suite pour vérifier que c'est compatible
...
finite space domains),
we find that the evolution of $B_k$ is:
\myTD{harmoniser les résultats}
\begin{equation}
