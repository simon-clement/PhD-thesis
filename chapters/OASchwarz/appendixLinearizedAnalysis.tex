...
We interest ourselves to the linearization of
\eqref{eq:SWRbulk} around a stationary state $U^e_j, \phi^e_j$.
\begin{equation*}
\begin{array}{rcll}
...
Finally, the jump of the solution across the interface is
\begin{equation*}
\widehat{u}_{a, 1/2} - 
...
+ A_k^1\left(1+\lambda_o\right)^{2M_o}
	:= B_k^1 I_a^{-} - A_k^1 I_o^{-}
\end{equation*}
and we also define $I_j^{+} = 2-I_j^{-}$. Note that all the variables
$I_j^{\pm}\rightarrow 1$ with the infinite domain hypothesis,
which is used in \eqref{eq:cvBk}.
...
\end{equation}
And the fluxes at the interface are obtained 
...
\begin{remark}
	...
	The convergence rate is then identical to $a_1$,
