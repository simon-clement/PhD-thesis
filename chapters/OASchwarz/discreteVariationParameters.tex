...
$\widehat{u}_{1/2} - \widehat{u}_{-1/2}$.
\myTD{Using a Laplace transform would avoid
having to separate the cases. I actually
think that with infinite
domains the function is not in $L^2([0,T], \mathbb{R})$
because its Fourier transform is affine (non-zero)
for $\omega=-f$}
\begin{itemize}
\item if $\omega=-f$:
	\begin{equation}
...
\end{itemize}
\myTD{Reste à montrer que \eqref{eq:OASchwarz_appendix_matrix_f} est
inversible}
